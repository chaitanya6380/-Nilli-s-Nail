        ...
        <div className="hidden md:flex space-x-10 items-center">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              href={link.path}
              className={`text-sm uppercase tracking-widest transition-colors hover:text-[#E7646A] ${pathname === link.path ? 'text-[#E7646A] font-medium' : 'text-gray-600'}`}
            >
              {link.name}
            </Link>
          ))}
          <Link href="/contact" className="bg-[#E7646A] text-white px-6 py-2 rounded-full text-xs uppercase tracking-widest hover:bg-[#d4545a] transition-all transform hover:-translate-y-0.5 shadow-md">
            Book Now
          </Link>
        </div>
