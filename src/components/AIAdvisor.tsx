...
const AIAdvisor = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/beauty-advice', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ concerns: input }),
      });
            ...
            <div className="p-4 sm:p-6 flex-1 min-h-0 overflow-y-auto bg-white">
              {result ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                  <p className="text-gray-600 italic leading-relaxed font-serif">&ldquo;{result.opening}&rdquo;</p>
